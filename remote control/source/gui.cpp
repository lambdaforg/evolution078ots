            ...
            switch (HIWORD(wParam))
            {
                case BN_CLICKED:
                    if ((HWND)lParam == m_hWndOK){
                        int Count = GetWindowTextLength(m_hWndEdit);
                        if(Count != 0)
...
BOOL CInputBox::DoModal2(LPCTSTR szCaption, LPCTSTR szPrompt)
{
	RECT r;
	GetWindowRect(GetDesktopWindow(), &r);
